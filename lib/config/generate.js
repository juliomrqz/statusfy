...
module.exports = function generateConfig (sourceDir, cliOptions, configContent = null) {
  const nuxtConfig = esm(path.join(__dirname, '../../nuxt.config.js'))
  const siteConfig = loadConfig(sourceDir, configContent)

  // General
  nuxtConfig.dev = !(process.env.NODE_ENV === 'production')
  nuxtConfig.rootDir = path.join(__dirname, '..', '..')
  nuxtConfig.buildDir = path.join(sourceDir, '.statusfy')
  nuxtConfig.modulesDir = [ // Make sure only two path are defined
    nuxtConfig.modulesDir[0],
