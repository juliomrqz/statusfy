...
const { sortBy, drop } = require('lodash')
...
const cache = LRU({
  max: 1000,
  maxAge: process.env.NODE_ENV === 'production' ? 1000 * 60 * 60 : 1
})
  ...
  return {
    ...
    incidentsHistory (lang, page = 1, pageSize = 3) {
      ...
      if (sortedIncidents.length > 0) {
        const start = moment(sortedIncidents[sortedIncidents.length - 1].date).startOf('month')
        const end = moment(sortedIncidents[0].date).startOf('month')
        const range = moment.range(start, end)
        const rangeMonths = Array.from(range.by('month', {excludeStart: true})).reverse()
        periods = rangeMonths.map((period, i) => { return { id: `${period.year()}-${period.month() + 1}`, incidents: [], order: i + 1, count: 0 } })
    ...
    incidentsTimeline (lang, daysNumber = 7) {
      ...
      for (let day of rangeDays) {
        ...
        for (let incident of sortedIncidents) {
          ...
          } else if (Math.abs(difference) > 24) {
            // Incidents are already ordered chronologically,
            // so if difference is more than 24 hours,
